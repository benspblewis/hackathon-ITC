...
const Home = ({ username, setUsername, room, setRoom, socket }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await axios.post("http://localhost:8080/chat/join-chat", {
      language: roomName,
    ...
    if (response) {
      ...
      navigate(
        `/chat/?username=${userName}&&userId=${currentUser.userId}&&roomId=${response.data.chatId}`
      );
    }
  };
        ...
        <Card backgroundColor="blue.50" paddingBottom={200}>
          ...
          <form className="home__container" onSubmit={handleSubmit}>
            <h2 className="home__header">
              Please choose a username and a language to start your chin wag
              now!
            </h2>
